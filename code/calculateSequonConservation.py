...
def calculateSequonConservation():
    print(">Calculating conservation for each sequon")
    ...
    results = Parallel(n_jobs=60, verbose=10)(
        delayed(calculateConservationForProtein)(rowIndex, row["index"],
                                                 multiz[multiz["id"] == row["ensId"]].reset_index(drop=True))
